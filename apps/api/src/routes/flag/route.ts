...
router.use("/:flagId", resolveRoleByFlagParams);
    ...
    try {
      ...
      const newFlag = await db.$transaction(async (tx) => {
        ...
        const environmentStatesData = project.environments.map((env) => ({
          ...
          enabled: false,
          defaultVariationId: offVariation.id,
        }));
  ...
  async (req: RequestWithSession, res: Response) => {
    ...
    try {
      ...
      if (!flagToDelete) {
        return res
          .status(404)
          .json({ error: "Flag not found or unauthorized." });
      }
